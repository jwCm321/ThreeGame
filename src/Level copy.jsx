...
const LaserBeam = (camera) => {
    ...
    // UseEffect to initialize the LaserBeam object when the component mounts
    // useEffect(() => {
    const newLaserBeam = new THREEx.LaserBeam();

    var laserCooked = new LaserCooked(newLaserBeam);

    newLaserBeam.object3d.position.x = -2;
    newLaserBeam.object3d.position.z = 0.2;
    newLaserBeam.object3d.position.y = 0.3;
   // newLaserBeam.object3d.scale.set(3.5, 1, 1);
    // }, []);
    ...
    useFrame((state, delta) => {
        if (newLaserBeam) {
            ...
            newLaserBeam.object3d.rotation.z = (Math.abs(rotationValue * Math.PI)) / 180;

            //laserCooked.update(delta, time)
        }

        // const box1 = new THREE.Box3().setFromObject(newLaserBeam);
        ...
        // mouse.x = (ballPosition.x / window.innerWidth) * 2 - 1;
        // mouse.y = -(ballPosition.y / window.innerHeight) * 2 + 1;

        // raycaster.setFromCamera(mouse,state.camera);
        // const intersects = raycaster.intersectObject(newLaserBeam.object3d);

        // if (intersects.length > 0)
        //     console.log('Collision detected!');
    });
    return <primitive object={newLaserBeam.object3d} />;
};
//LBC End

export function BlockLaser({ position = [0, 0, 0] }, camera = { camera }) {
    // const obstacle = useRef()
